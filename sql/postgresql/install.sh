...
if [ -e ${DIR}/db.properties ]; then
  ...
  RESULT=`psql -h ${DB_HOSTNAME} -q -p ${DB_PORT} -U postgres -d postgres --tuples-only --command "SELECT spcname FROM pg_tablespace WHERE spcname = 'cpf';"`
  if [ "$?" != "0" ]; then
    echo ERROR: Psql executed with error code $?
    exit
  fi
  if [ " cpf" == "${RESULT}" ]; then
    ...
    echo CREATE TABLESPACE cpf OWNER cpf LOCATION \'${TABLESPACE_DIR}\' | psql -h ${DB_HOSTNAME} -p ${DB_PORT} -U postgres -d postgres
    if [ "$?" != "0" ]; then
      echo ERROR: Psql executed with error code $?
      exit
    fi
  ...
  RESULT=`psql -h ${DB_HOSTNAME} -q -p ${DB_PORT} -U postgres -d postgres --tuples-only --command "SELECT datname FROM pg_database WHERE datname = '${DB_NAME}';"`
  if [ "$?" != "0" ]; then
    echo ERROR: Psql executed with error code $?
    exit
  fi
  ...
  if [ "$?" != "0" ]; then
    ...
    psql -h ${DB_HOSTNAME} -p ${DB_PORT} -U cpf -d ${DB_NAME} -f cpf-ddl-all.sql > cpf-ddl.log 2>&1
    if [ "$?" != "0" ]; then
      echo ERROR: Psql executed with error code $?
      exit
    fi
